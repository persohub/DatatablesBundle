    ...
    public function setWhere(QueryBuilder $qb)
    {
        ...
        if ($this->requestParams['sSearch'] != '') {
            ...
            for ($i = 0; $i < $this->requestParams['iColumns']; $i++) {
                if (isset($this->allColumns[$i]) && isset($this->requestParams['bSearchable_' . $i]) && $this->requestParams['bSearchable_' . $i] === 'true') {
        ...
        for ($i = 0; $i < $this->requestParams['iColumns']; $i++) {
            if (isset($this->allColumns[$i]) && isset($this->requestParams['bSearchable_' . $i]) && $this->requestParams['bSearchable_' . $i] === 'true' && $this->requestParams['sSearch_' . $i] != '') {
                $searchField = $this->allColumns[$i];
